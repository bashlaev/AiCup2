...
public final class BetterSolver extends CallableSolver {
	...
	private static boolean isNavigable(CellData[][] cells,
	...
	private static boolean isNavigableFrom(CellData[][] cells, int x, int y,
			TrackCell from) {
		if (!isNavigable(cells, x, y)) {
			return false;
		}
		if (from == TrackCell.PRE_START_LINE
				&& START_CELLS.contains(cells[x][y].getType())) {
			return false;
		}
		return true;
	}
	...
	private static CellData safeCellAt(CellData[][] cells,
			Pair<Integer, Integer> coords) {
	...
	private static boolean isIntersectionLegal(CellData[][] cells,
			Intersection i) {
		if (i instanceof Vertex) {
			Pair<Integer, Integer> coords = ((Vertex) i).getCoords();
	...
	private static Set<Pair<Integer, Integer>> cellsAtDistance(
			CellData[][] cells, int cellX, int cellY, int distance) {
		...
		for (int x = cellX - distance; x <= cellX + distance; x++) {
			for (int y = cellY - distance; y <= cellY + distance; y++) {
				if (Math.abs(x - cellX) + Math.abs(y - cellY) == distance) {
					...
					if ((cellData != null)
							&& (cellData.getType().isNavigable())
							&& (isMoveLegal(cells, Pair.of(cellX, cellY),
									Pair.of(x, y)).getLeft())) {
	...
	public BetterSolver(final int depth) {
		this.depth = depth;
	}
	...
	private static class Move {
		...
		Move(final Pair<Integer, Integer> dest, final int value,
	...
	public Callable<String> solverFor(final String task) {
		return new Callable<String>() {
			...
			public String call() throws Exception {
				...
				CellData[][] cells = new CellData[track.getWidth()][track
						.getHeight()];
				for (int x = 0; x < cells.length; x++) {
					for (int y = 0; y < cells[0].length; y++) {
				...
				fillDistances(cells);
				System.out
						.println("Distance to finish calculated for each cell");
				for (int y = 0; y < cells[0].length; y++) {
					for (int x = 0; x < cells.length; x++) {
						System.out
								.print(cells[x][y].type.isNavigable() ? String
										.format("% 4d",
												cells[x][y].distanceToFinish)
										: "   -");
						System.out.print(" | ");
					}
					System.out.println();
				...
				while (true) {
					// choose best move from pos
					Move bestMoveCombo = getBestMove(track, cells, pos, speed,
							depth);
					Pair<Integer, Integer> bestMove = bestMoveCombo.dest;
					moves.add(bestMove);
					speed = Pair.of(bestMove.getLeft() - pos.getLeft(),
							bestMove.getRight() - pos.getRight());
					pos = bestMove;
					solutionBuilder.append(bestMoveCombo.character);
					if (bestMoveCombo.value > 0) {
						// finishing move
						break;
					}
				}
				System.out.println("Finished in " + (moves.size() - 1)
						+ " moves");
